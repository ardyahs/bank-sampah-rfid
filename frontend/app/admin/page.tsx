...
export default function AdminPage() {
  ...
  const [tab, setTab] = useState<
    "warga" | "tambah-warga" | "produk" | "manual" | "scan" | "transaksi" | "leaderboard" | "kontak"
  >("warga");
  ...
  const tabs: { key: typeof tab; label: string; icon: LucideIcon }[] = [
    ...
    { key: "scan", label: "Scan Masuk", icon: Nfc },
        ...
        {tab === "scan" && (
          <section className="space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center gap-2">
              <div>
                <h2 className="eco-title flex items-center gap-2">
                  <Nfc className="w-5 h-5 text-primary" /> Scan Kartu Masuk
                </h2>
                <p className="text-sm text-gray-500">
                  Warga tempel kartu di alat &rarr; muncul di sini. Pilih jenis sampah &amp; isi berat, lalu klik Proses.
                </p>
              </div>
              <span className="text-xs text-gray-400 sm:ml-auto">Auto-refresh tiap 4 detik</span>
            </div>

            {pendingScans.length === 0 && (
              <div className="eco-card p-6 text-center text-gray-400 text-sm">
            ...
            {pendingScans.map((ps) => {
              ...
              return (
                <div key={ps.id} className="eco-card p-4 grid sm:grid-cols-4 gap-3 items-center">
                  <div>
                    <p className="font-medium text-gray-800">{ps.nama_kepala_keluarga}</p>
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => prosesScan(ps.id)}
                      className="eco-btn-primary flex-1 py-2 text-sm">
                      Proses
                    </button>
                    <button onClick={() => batalScan(ps.id)}
                      className="inline-flex items-center justify-center px-3 py-2 rounded-lg text-sm bg-red-50 text-red-600 border border-red-200 hover:bg-red-100 transition">
                      Batal
                    </button>
                  </div>
                </div>
